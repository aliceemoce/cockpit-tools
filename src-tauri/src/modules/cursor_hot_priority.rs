...
const MULTI_PRIORITY_SHIM: &str = r#"/*cockpit-pri-s*/(function(){try{if(window.__cockpitPri)return;window.__cockpitPri=1;function applyAuth(){try{var a=window.__cockpitSeamlessAuth;if(!a||!a.accessToken||!window.store||typeof window.store.set!=="function")return;window.store.set("cursorAuth/accessToken",a.accessToken,-1);if(a.refreshToken)window.store.set("cursorAuth/refreshToken",a.refreshToken,-1);if(a.email)window.store.set("cursorAuth/cachedEmail",a.email,-1);if(a.machineIds){if(a.machineIds.devDeviceId)window.store.set("telemetry.devDeviceId",a.machineIds.devDeviceId,-1);if(a.machineIds.machineId)window.store.set("telemetry.machineId",a.machineIds.machineId,-1);if(a.machineIds.macMachineId)window.store.set("telemetry.macMachineId",a.machineIds.macMachineId,-1);if(a.machineIds.sqmId)window.store.set("telemetry.sqmId",a.machineIds.sqmId,-1);}}}catch(_e){}}var _of=window.fetch.bind(window);window.fetch=function(input,init){var u=typeof input==="string"?input:(input&&typeof input.url==="string"?input.url:"");var p=_of(input,init);if(u.indexOf("/api/get-token")<0)return p;return p.then(function(r){if(!r||!r.ok)return r;return r.clone().json().then(function(data){try{var a=window.__cockpitSeamlessAuth;var t=(a&&a.accessToken)||(window.store&&window.store.get("cursorAuth/accessToken",-1));var e=(a&&a.email)||(window.store&&window.store.get("cursorAuth/cachedEmail",-1));var rt=(a&&a.refreshToken)||(window.store&&window.store.get("cursorAuth/refreshToken",-1));if(t&&t!==""&&t!=="undefined"){data.accessToken=t;if(rt&&rt!==""&&rt!=="undefined")data.refreshToken=rt;if(e&&e!==""&&e!=="undefined")data.email=e;if(a&&a.machineIds)data.machineIds=a.machineIds;data.is_new=false;applyAuth();}}catch(_e){}return new Response(JSON.stringify(data),{status:200,headers:{"Content-Type":"application/json"}});}).catch(function(){return r;});});};applyAuth();if(!window.__cockpitPriTimer){window.__cockpitPriTimer=setInterval(applyAuth,1000);}}catch(_e){}})();/*cockpit-pri-e*/"#;
