...
1. **制作程序时必须参考这四个程序**：写切号、换号、无感、额度显示、自动选号等逻辑前，先看这四个程序是怎么做的。
2. **出问题从不出问题的地方找**：Cockpit 某条链路不通时，先去看这四个程序里同一功能不出问题的地方是怎么实现的，找到差异点或直接复刻代码。
3. **直接复刻代码**：如果某个程序的做法已验证可用，Cockpit 可以直接复刻它的代码路径（注意技术栈差异：Python/Rust/JS 转换）。
...
5. **四个程序都要做成自动更新/自动安装/自动覆盖**：
   ...
   - 三个外部程序：工作区副本（`external/xubei_and_renewal/`）要能自动从源头更新/覆盖（当源头有新版本时，自动复制覆盖工作区副本）
...
- 禁止不参考这四个程序就自己从头写切号/换号逻辑（已有成熟实现不参考 = 重复造轮子）
- 禁止删除无忧小助手已对齐的切号逻辑
